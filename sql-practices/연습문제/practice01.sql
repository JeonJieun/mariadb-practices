...
-- 문제2. 전체직원의 다음 정보를 조회하세요. 가장 선임부터 출력이 되도록 하세요. 출력은 이름, 성별, 입사일 순서이고 “이름”, “성별”, “입사일"로 컬럼 이름을 대체해 보세요.
select concat(first_name, ' ', last_name) as '이름', gender as '성별', hire_date as '입사일'
from employees
order by hire_date asc;

-- 문제3. 여직원과 남직원은 각 각 몇 명이나 있나요? -- 집계
select if(gender = 'F', '여', '남') as '성별', count(*) as '수'
from employees
group by gender;

-- 문제4. 현재 근무하고 있는 직원 수는 몇 명입니까? (salaries 테이블을 사용합니다.)
select count(*) as '현재 근무 직원 수'
from salaries
...
-- 문제6. 현재 부서 매니저는 몇 명이나 있나요?(역임 매니저는 제외)
...
select count(*) as '현재 급여가 120,000이상 받는 사원 수'
from salaries
...
select count(*) as '현재 Engineer 직책의 사원 수'
from titles
where title = 'Engineer' and to_date = '9999-01-01';

-- 문제11. 사번이 13250(Zeydy)인 직원의 직책 변경 상황을 시간순으로 출력해보세요.
select title as '직책', from_date '부터', to_date '까지'
from titles
where emp_no = 13250

...
select emp_no, max(salary) as '최고임금', 
min(salary) as '최저임금',
...
select date_format(cast(max(hire_date) as date), '%Y년 %m월 %d일') as '마지막으로 신입사원이 들어온 날'
from employees;

-- 문제3. 가장 오래 근속한 직원의 입사일은 언제인가요? 다음 형식으로 출력해주세요. 예) 2014년 07월 10일
select date_format(cast(min(a.hire_date) as date), '%Y년 %m월 %d일') as '가장 오래 근속한 직원의 입사일'
...
select TIMESTAMPDIFF(YEAR, min(a.birth_date), curdate())+1 as '최 연장자 나이',
TIMESTAMPDIFF(YEAR, max(a.birth_date), curdate())+1 as '최고 어린 사원'
